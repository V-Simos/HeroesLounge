...
- **forceSecure=1 (account page only):** RainLab `Account::onRun()` 302s any plain-http non-AJAX GET → in dev, `/user` will redirect to `https://…` and fail. **Committed value MUST stay `forceSecure = 1`.** For live dev verification, temporarily set `0`, verify, restore BEFORE commit (Task 8 re-checks the committed value via `git show`). Document in `dev/README.md`.
- **Auth for verification:** frontend login is by EMAIL. Use the Task-7 tinker user: `docker compose -f dev/docker-compose.yml exec -T web php artisan tinker` → `$u = RainLab\User\Models\User::find(41); $u->password = 'dev12345'; $u->password_confirmation = 'dev12345'; $u->forceSave();` then log in with `$u->email` / `dev12345`. (Look up the email via tinker; do not guess.)
...
- [ ] **Step 1: Page front-matter VERBATIM** (incl. `forceSecure = 1`): `title = "Account"`, `url = "/user/:code?"`, `layout = "default"`, `[SlothAccount] redirect = "user/account"`, `paramCode = "code"`, `forceSecure = 1`. Body `{% component 'SlothAccount' %}`. **The `redirect = "user/account"` property resolves a CMS page by file name — our page at `pages/user/account.htm` satisfies it.** For dev verification ONLY, flip `forceSecure = 0` locally; NEVER commit the flip.
...
- [ ] **Step 6: Verify live** (forceSecure temp 0): (a) `/user` guest → two themed panels, dialog opens/closes, keyboard: dialog is natively focus-trapped, Esc closes; (b) register submit with empty/invalid fields → server validation flash errors render (no Discord API needed for validation-path checks); full happy-path register is **dev-blind** (Discord membership check + mail) — document; (c) sign in as the tinker user (EMAIL login) → redirects to `/user` account view (authed branch renders — plugin Bootstrap update partial until T3, expected); (d) `activation_check` only shows for non-activated users (tinker: check `is_activated` on a spare user to exercise both branches if quick); (e) responsive + logs/console hygiene; (f) **restore `forceSecure = 1` and verify with `git diff` before committing**.
