...
- [ ] **Step 3: Media tab.** Avatar + Banner forms: `form_open({... files: true})`, keep the FULL `selectFile.js` contract (`.fileselect` wrapper, keyboard-focusable visually transparent native file input named `avatar`/`banner` with `accept="image/png"` and a visible focus indication, readonly text sibling, `#avatarUploadError`/`#bannerUploadError`, helper copy verbatim); preview `<img>` from `__SELF__.sloth.user.avatar.path` / `__SELF__.sloth.banner.path` with the Task-5 `onerror` neutral-placeholder pattern (code-commented).
