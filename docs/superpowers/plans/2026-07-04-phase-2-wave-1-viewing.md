...
- [ ] **Step 5: Capture SPIKE FINDINGS + revert the scratch page.**
  ...
  > - **Task 4 port basis (the callback JS is NOT in the repo — it lived on the reverted scratch page):** port `hlToggleSpoilers(show)` from the frozen old-theme spoiler callbacks — `themes/HeroesLounge-Theme/pages/playoff/view.htm` (`showHideSpoilersPlayoffView`) and `themes/HeroesLounge-Theme/pages/season/playoff.htm` (`showHideSpoilersSeasonPlayoff`) (both function names verified present 2026-07-05) — retargeted onto the new `.hl-*` hooks (`.hl-name` / `.hl-tbd` / `.hl-logo` / `.hl-score` / `.is-masked` / `.bracket-losers`). The data-walk itself (seenTeams tracking, splice-on-`BYE!`, `.bracket-losers` exclusion) is described in the bullet above.
