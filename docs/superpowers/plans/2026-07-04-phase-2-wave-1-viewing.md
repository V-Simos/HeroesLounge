...
- [ ] **Step 5: Capture SPIKE FINDINGS + revert the scratch page.**
  ...
  > **SPIKE FINDINGS (fill during execution, then commit this note):**
  > - Chosen render approach: _[component-override at `partials/PlayoffOverview/default.htm` re-emitting frozen offsets/polylines + `#double_elim` scroll wrapper — confirm]_
  > - CSS technique that avoided the `heroeslounge.css` clobber: _[fresh `.hl-bracket` ancestor scope / new class names — record what worked]_
  > - Spoiler data-walk that reproduced repeat-team hiding: _[record]_
  > - Bracket types visually verified: _[list, with fixture ids]_
  > - Known residue (legacy ResizeSensor/ElementQueries JS injected, harmless): _[note]_
